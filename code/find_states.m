function train_data=find_states(train_data)
    fprintf('In find_states.m \n');
    ...
    for i=1:numel(train_data)
        ...
        for t=1:numel(train_data(i).trial)
            ...
            %plot( locs, pks>0, 'k*');
            %title(['# ',num2str(t),' of ',num2str(numel(train_data(i).trial)),' trials for dataset ',num2str(i),' : ',train_data(i).name]);
            %tname=strcat('../plots/States_D',num2str(i),'_T',num2str(t),'.jpg');
            %print ('-djpeg','-r72',tname);
