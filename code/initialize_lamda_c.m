function [lamda]=initialize_lamda_c(codebook,ii,jj)
    lamda=[];
    for i=1:numel(codebook)
        ...
        %B=zeros(codebook(i).num_code,num_states);
        %C=zeros(1,codebook(i).num_states);
        C=[1 zeros(1,codebook(i).num_states-1)];
        nobs=zeros(codebook(i).num_code,num_states);
